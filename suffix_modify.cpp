...
int node ::edgeChange(string s, int str_num, int sub_num, int pos)
{
    ...
    int i = 0;
    common = s[0];
    for (i = 1; i < min(str[pos].size(),s.size()); i++)
    {
        if (s[i] == str[pos][i])
        {
            common += s[i];
                ...
                if(t->checkSplit(v) == -1){
                    ...
                    if(t->child[l] != NULL){
                        ...
                        int m;
                            string common;
                            common = v[0];
                            for(m=1;m<min(t->str[l].size(),v.size());m++){
                                if(v[m] == t->str[l][m]){
                                    common += v[m];
                                }
                                else{
                        ...
                        string s1,s2;
                        s1 = (t->str[l]).substr(common.size());//at node
                        s2 = v.substr(common.size());//new

                        if(s1.size() == 0){
                            v = s2;//v.substr((t->str[l]).size())
                            t = t->child[l];
                        }
                        else{
                            t->str[l] = common; 
                            node* t1,*t2;
                            t2 = t->child[l];
                            t1 = new node(s1,t->num_rank[l]);
                            vector<pair<int,int>> ip;
                            ip.push_back(make_pair(j,i));
                            t1->addStr(s2,ip);//v.substr(common.size()) is s2
                        ...
                        // if((t->str[l]).compare(v.substr(0,t->str[l].size())) == 0){
                        ...
                        //     string next = (t->str[l]).substr(common.size());
                        //     t->str[l] = common; 
                        //     node* t1,*t2;
                        //     t2 = t->child[l];
                        //     t1 = new node(next,t->num_rank[l]);
                        //     vector<pair<int,int>> ip;
                        //     ip.push_back(make_pair(j,i));
                        //     t1->addStr(v.substr(common.size()),ip);
